...
**升级后重新上传旧日志包**，才能补齐新字段并修正原索引的 Pod。旧数据保留并给出提示，不自动重写。Agent 终端需要新建会话，才能使用更新后的 Skill。实际样例验证记录见 [SAMPLE_VALIDATION.md](docs/SAMPLE_VALIDATION.md)。
