...
停机错过 02:00 会在下次启动补检查；有导入、查询或 AI 任务时延期 15 分钟。清理压缩期间页面会提示暂缓查询，完成后自动刷新日志包列表。升级可以继续使用原来的 `data`，无需提前重新导入；旧数据没有导入完成时间时按原创建时间判定，因此已有过期索引可能在启动补检查时被清理。
...
升级到 v2.1：EXE 用户关闭旧程序后从 `exe` 分支或 Release 替换 `LogScope.exe`；源码用户停止旧服务 → `git pull` → `python -m pip install -r requirements.txt` → `python app.py` → 刷新页面。**可以复用原有 data；未过期日志不必重新导入，旧终端历史保留。** v2.0 的原生聊天、文件式模型配置、只读工具循环、Git 同步与组内共享启动保持不变，v2.1 新增过期索引自动清理。原生对话首次使用需填写 `data/ai-config.json`；不会复用或提取 CLI 登录凭据。EXE 只发布到独立的 `exe` 分支和 Release，`main` 保持纯源码。
...
- 自动清理：导入完成满 72 小时的日志记录、解析元数据和检索索引，在每日服务电脑本地 02:00 清理。原始 ZIP、过期归档清单、AI 会话与配置保留；数据库压缩后实际归还磁盘空间。运行中的任务会延后清理，维护期间暂时暂停日志查询。详情见 [自动清理说明](docs/RETENTION.md)。
