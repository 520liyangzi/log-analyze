...
6. 如果需要继续定位代码，在报告下填写本机 Git 项目绝对路径、读取并选择分支。页面会先预览 `code-task.md`；确认后，同一个 AI 使用只读工具检查固定 commit 并更新报告，不会切换或修改你的工作区。
...
启动 AI 前必须预览完整任务；**仅预览 task.md** 不会启动任何进程，**查看本次任务** 可核对实际创建的版本。`task.md` 已带日志包范围和从问题提取的接口/流水号线索，AI 默认直接组合条件查询现有 SQLite/FTS 索引，不重复解压或建索引。单次搜索通常很快；总耗时主要来自 AI 启动、模型推理、多轮上下文/原包核验和报告生成。输入框中的问题是下一次新任务的草稿，修改它或切换左侧日志包，不会改变正在运行的会话。最多同时运行 3 个终端；刷新网页可重连，停止服务会结束进程，历史文件仍在磁盘。
...
升级到 v1.5：停止旧服务 → `git pull` → `python -m pip install -r requirements.txt` → `python app.py` → 刷新页面。**已有日志不必重新导入**；启动时会自动补充接口 URL 索引。已导入的日志与本机配置保留；以后升级代码也不会覆盖你在页面保存的规则。
