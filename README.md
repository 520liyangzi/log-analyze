...
升级到 v1.17：停止旧服务 → `git pull` → `python -m pip install -r requirements.txt` → `python app.py` → 刷新页面。**已有日志不必重新导入，搜索、AI 配置和历史任务都不受影响。** 新导入采用批量写入、受控 WAL 和不重复保存原文的紧凑全文索引；v1.16 修复了跨代索引导致的搜索变慢和删除失败。v1.17 将删除改为默认快速模式，不再为删除一个包而重写整个数据库；确实需要让 `logs.sqlite3` 立即缩小时，可在删除弹窗勾选“立即归还操作系统磁盘空间”。
