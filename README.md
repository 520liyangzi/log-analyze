...
升级到 v1.8：停止旧服务 → `git pull` → `python -m pip install -r requirements.txt` → `python app.py` → 刷新页面。**已有日志不必重新导入**；已有 AI 任务也会出现在历史窗口中（旧任务没有此前未记录的终端正文和 Session ID）。已导入的日志与本机配置保留。
