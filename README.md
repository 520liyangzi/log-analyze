...
**排查窗口**支持同时运行 3 个任务和不限数量的历史任务。任务状态、完整 `terminal.log`、`task.md`、报告和 AI Session ID 都保存在本机。选择已结束的历史窗口时不会自动显示以前的 CMD 输出；点击 **查看终端记录** 才会回放，点击 **恢复这个对话** 才会调用 AI 的会话恢复。公司工具的默认恢复模板是 `{command} --sessions {session_id}`；标准 Claude 可改成 `{command} --resume {session_id}`。
...
升级到 v1.9：停止旧服务 → `git pull` → `python -m pip install -r requirements.txt` → `python app.py` → 刷新页面。**已有日志不必重新导入**；已导入的日志、本机 AI 配置和历史任务都会保留。
