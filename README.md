...
升级到 v1.15：停止旧服务 → `git pull` → `python -m pip install -r requirements.txt` → `python app.py` → 刷新页面。**已有日志不必重新导入，搜索、AI 配置和历史任务都不受影响。** 新导入采用批量写入、受控 WAL 和不重复保存原文的紧凑全文索引，导入过程会显示已处理数据量与当前速度；在线采集的平台地址、用户名和密码调整为基础必填项。
