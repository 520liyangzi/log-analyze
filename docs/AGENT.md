...
3. 填写问题，例如“检查 /api/model/map 为什么慢，看看不同 Pod 是否有异常”，点击 **生成任务并预览**。核对完整 `task.md` 后，再点 **确认并启动 AI**。
...
`report.md` 生成后，页面显示第二阶段入口。填写 Git 项目绝对路径，点击读取分支并选择本地可见的分支；预览 `code-task.md` 后确认发送。工具把分支解析成固定 commit，因此不会 checkout，也不会影响未提交修改。AI 通过 `tools/project.py grep/show/tree/log` 查已提交代码，将文件、行号和判断追加到原报告。分支在预览后发生移动时，页面会要求重新预览。

这一阶段只读代码，不执行构建、测试和项目脚本。若要分析未提交改动，需要先自行提交到一个分支；远程最新分支也需要先在项目目录完成 `git fetch`，LogScope 不会替你联网更新仓库。

切换左侧日志包不会改变正在运行的任务。分析新问题时创建新任务，或者在终端里明确追问。页面上方的排查窗口可切换任务，最多同时运行 3 个真实进程，已结束的历史任务不限数量；切走窗口不会结束后台任务。任务、终端记录和报告刷新后仍可查看。
...
升级到 v1.6 不需重传日志。旧 AI 任务会显示在历史窗口，但升级前没有记录的终端正文和 Session ID 无法补回。v1.1 或更早的数据若显示解析器升级提示，需要重新上传以补齐字段；v1.0 未保留原 ZIP 的数据也需要重新上传才能原包核验。
