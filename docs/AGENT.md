...
选择历史排查窗口只显示任务概览，不会自动回放以前的 CMD 输出。点击 **查看终端记录** 后，按钮会变成 **隐藏终端记录**；再次点击、切换任务或恢复 AI 对话都会退出回放状态。

历史任务没有自动识别 Session ID 时，直接将 UUID 粘贴到 **AI Session ID** 输入框，再点 **恢复这个对话** 即可，页面会在恢复时使用并保存该 ID。只有真正请求恢复期间按钮才显示“正在恢复…”。
