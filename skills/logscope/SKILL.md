...
如果当前目录有 `task.md`，先读取它。它包含本次问题、日志包范围、查询线索、分析规则版本和工具说明；使用 task.json 指定的 `logscope_command` 查询。已有任务使用自己的规则快照，仅在用户明确发送规则更新时切换，不自行用最新规则覆盖它。优先使用已有范围和线索直接组合查询，不重复无目的调用 datasets/files，不重新解压或建立索引。
